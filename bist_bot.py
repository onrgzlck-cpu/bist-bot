...
# 🔴 HİSSE EKLEME ALANI: İstediğiniz tüm hisseleri bu listenin içine ekleyebilirsiniz.
TUM_HISSELER = [
...
# Ekranın sıfırlanmasını engellemek için tarama sonuçlarını hafızada tutuyoruz
if "tarama_df" not in st.session_state:
    st.session_state.tarama_df = None

# --- SOL MENÜ (FİLTRELER VE SEÇİMLER) ---
...
if hepsini_sec:
    ...
    secilen_hisseler = st.sidebar.multiselect("Hisseleri Seçin", TUM_HISSELER, default=TUM_HISSELER[:2])
...
def detayli_analiz_yap(ticker):
    try:
        ...
        if h1 < l3:
            ...
            # Potansiyel Kâr Hesaplama: ((Hedef - Giriş) / Giriş) * 100
            kar_hesap = ((hedef_yeri - giris_yeri) / giris_yeri) * 100
            kar_orani = f"%{round(kar_hesap, 2)}"
        ...
        return {
            ...
            "Güncel Fiyat (TL)": round(son_fiyat, 2),
            "SMC Durumu": "SMC Alım (FVG)" if h1 < l3 else "Normal",
            "SMC Giriş Seviyesi": giris_yeri if giris_yeri > 0 else "-",
            "SMC Hedef Seviyesi": hedef_yeri if hedef_yeri > 0 else "-",
            "Tahmini Kâr Potansiyeli": kar_orani,
            "RSI (14)": round(rsi, 2),
...
# --- TARAMA TETİKLEME BUTONU ---
if st.button("📡 Piyasayı Tara ve Radar Başlat"):
    sonuclar = []
    with st.spinner("BIST Güncel Verileri Çekiliyor..."):
    ...
    if sonuclar:
        st.session_state.tarama_df = pd.DataFrame(sonuclar)
    else:
        st.error("Seçilen hisselerden veri çekilemedi.")

# --- HAFIZADAKİ VERİYİ EKRANA BASMA (RESET ENGELLEYİCİ) ---
if st.session_state.tarama_df is not None:
    main_df = st.session_state.tarama_df
    
    # Filtreleme seçeneğini uygula
    if gosterim_tipi == "Sadece Sinyal Verenler":
        gosterilecek_df = main_df[main_df["SMC Durumu"] == "SMC Alım (FVG)"]
    else:
        gosterilecek_df = main_df
        
    st.subheader("📊 Akşam Analiz Matrisi")
    ...
    # --- TEKİL HİSSE DETAY VE GRAFİK BÖLÜMÜ ---
    st.subheader("🔍 Tekil Hisse Grafik İnceleme")
    
    # Seçim kutusu değiştiğinde artık sayfa sıfırlanmayacak, çünkü tablo yukarıda hafızadan okunuyor
    secilen_detay_hisse = st.selectbox(
        "Grafiğini görmek istediğiniz hisseyi seçin:", 
        gosterilecek_df["Hisse"].unique()
    )
    ...
    if secilen_detay_hisse:
        ...
        fig.update_layout(
            title=f"{secilen_detay_hisse} Güncel Teknik Grafiği", 
